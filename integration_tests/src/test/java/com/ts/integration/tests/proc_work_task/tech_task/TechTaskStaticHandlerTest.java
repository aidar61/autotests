...
public class TechTaskStaticHandlerTest extends BaseIntegrationTest {
    ...
    public void changePrgArea() {
        ...
        ApiAsserts.assertThat(techTaskController.getResponse()).isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK).checkingResponseMessageField("mstatusName", "Связь с ККПО").isParseableBody(TaskResponseBody.class).assertTask();
    ...
    public void changeCodeReview() {
        ...
        udf.setUdfMemo(generateUdfMemo(UDF_CDP_STEPPROGRESS, "[{\"id\":\"818181698c1b5016018c236e46604112\",\"name\":\"Предварительный анализ\",\"order\":0,\"taskId\":\"" + task.getId() + "\",\"weight\":0,\"budget\":7200,\"progress\":0,\"description\":\"\",\"status\":\"ACTUAL\",\"workTypeId\":\"402881c2516c220101516c711ff80024\",\"workTypeNorm\":2.0,\"hrs\":0,\"deletable\":true,\"actualBudget\":0,\"planby\":\"budget\",\"workTypeDraftAsString\":\"-\",\"workTypeAsString\":\"[0701] Иное\",\"draftChanged\":true}]"));
        task.refreshUdf(udf);
    ...
    public void taskDependOtherTask() {
        ...
        var response = techTaskController.getResponse();
        ApiAsserts.assertThat(response).isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK).assertTask();
    ...
    public void changeAuthor() {
        ...
        ApiAsserts.assertThat(techTaskController.getResponse()).isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK).isParseableBody(TaskResponseBody.class).assertTask();
        var taskDetail = apiController.receiveTask(task.getNumber());
        CommonAssert.assertThat(taskDetail).isCorrectSubmitterUser(creator2.getLogin());
    }
}
