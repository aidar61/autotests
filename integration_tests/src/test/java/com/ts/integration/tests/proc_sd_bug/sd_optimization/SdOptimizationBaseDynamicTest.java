package com.ts.integration.tests.proc_sd_bug.sd_optimization;
...
import com.ts.common.controllers.sdbug.SdOptimizationController;
...
import static com.ts.common.enums.TaskStatuses.STATUS_SDBUG_CLOSED;
import static com.ts.common.utils.InitEntities.*;
import static com.ts.common.utils.RandomUtils.*;

public class SdOptimizationBaseDynamicTest extends BaseIntegrationTest {
    public SdOptimizationController sdOptimizationController;
    ...
    public void beforeClass() {
        sdOptimizationController = apiController.getSdOptimizationController();
        ...
        task = InitEntities.getGeneralTask(TaskType.SD_OPTIMIZATION, Operations.CAT);

        var parentPayloadResponse = sdOptimizationController.getParentPayload(parent.getNumber(), "CAT_SDOPTIMIZATION");
    ...
    public void createTask() {
        ...
        task.setName("CAT_SDIMPROVE: " + generateComment());
        ...
        task.refreshUdf(udf);
        sdOptimizationController.create(task);
        var response = sdOptimizationController.getResponse();
        ApiAsserts
                .assertThat(response)
    ...
    public void taskAnalyze() {
        ...
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, ANALIZE);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .isParseableBody(TaskResponseBody.class)
                .assertTask()
    ...
    public void taskDecline() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, DECLINE);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskReturn() {
        ...
        udf.setUdfUser(generateUdfUser(STDT_HANDLER, SUPPORT_MEMBER));
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, RETURN);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskStart() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, START);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskCloseWoPatch() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, CLOSE_WO_PATCH);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskUndoClose() {
        ...
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, UNDO_CLOSE);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskUndoStart() {
        ...
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, UNDOSTART);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskRequestInfo() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, REQUESTINFO);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskProvideInfo() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, PROVIDEINFO);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskHotFix() {
        ...
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, HOTFIX);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskAcceptHotfix() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, ACCEPTHOTFIX);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskInPatch() {
        ...
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, INPATCH);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskUndoInPatch() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, UNDOINPATCH);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskSendPatch() {
        ...
        udf.setUdfList(generateUdfList(UDF_SDFEATURE_DOCREVISION, DOC_REVISION_YES));
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, SENDPATCH);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    public void taskClose() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, CLOSE);
        ApiAsserts
                .assertThat(sdOptimizationController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK)
                .assertTask()
    ...
    @Test(groups = {"PROC_SDBUG", "Regression"}, description = "Закрыть", dependsOnMethods = "taskClose")
    public void taskClose2() {
        apiController.updateToken(generateAuthToken(CLIENT));
        task.refreshTask();
        task.setResolution(generateResolution(Resolutions.QUESTION_NOT_RELEVANT));
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdOptimizationController.performCommonOperation(task, CLOSE);
        ApiAsserts
