...
public class SdTechAssistBaseStaticTest extends BaseIntegrationTest {
    ...
    @Test(groups = {"PROC_SDHELP", "Regression"}, description = "Задать вопрос (КЛИЕНТ)"
            , dependsOnMethods = "catSdTechAssist")
    void cliComment() {
        apiController.updateToken(generateAuthToken(CLIENT));
        udf = refreshUdf();
        task.refreshTask();
