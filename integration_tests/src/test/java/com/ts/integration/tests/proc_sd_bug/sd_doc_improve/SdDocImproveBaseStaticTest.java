...
public class SdDocImproveBaseStaticTest extends BaseIntegrationTest {
    ...
    public void taskAsk() {
        ...
        udf = refreshUdf();
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, CLI_COMMENT);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskChangePriority() {
        ...
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, CHANGE_PRIORITY);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskOurComment() {
        ...
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, OUR_COMMENT);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskChangeAttrs() {
        ...
        udf.setSecondUdfString(generateUdfString(UDF_SD_INITPERSON, generateString()));
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, CHANGE_ATTR);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskAppointCliWatchers() {
        ...
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, ADD_CLIENT_WATCHERS);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskChangeSdModule() {
        ...
        udf.setUdfTask(generateUdfTask(UDF_SD_MODULE, CORE));
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, CHANGE_MODULE);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskChangeLinkedTask() {
        ...
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, CHANGE_LINKED_TASKS);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskAddTrustWatcher() {
        ...
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, ADD_TRUST_WATCHER);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskAddWatcher() {
        ...
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, ADD_WATCHERS);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskPrivateComment() {
        ...
        task.refreshUdf(udf);
        sdDocImproveController.performCommonOperation(task, PRIVATE_COMMENT);
        ApiAsserts
                .assertThat(sdDocImproveController.getResponse())
                .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
    }
    ...
    public void taskChangeAuthor() {
        var authors = sdDocImproveController.receiveAuthor(task.getNumber());
        if (authors != null) {
            apiController.updateToken(generateAuthToken(SUPPORT_MANAGER));
            task.refreshTask();
            udf = refreshUdf();
            ...
            task.refreshUdf(udf);
            sdDocImproveController.performCommonOperation(task, CHANGE_AUTHOR);
            ApiAsserts
                    .assertThat(sdDocImproveController.getResponse())
                    .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
        }
    ...
    public void taskReassign() {
        ...
        if (authors != null) {
            ...
            udf.setThirdUdfUser(generateUdfUser(UDF_WATCHER, RANDOM_WATCHER));
            task.refreshUdf(udf);
            sdDocImproveController.performCommonOperation(task, CHANGE_RES_PERSON);
            ApiAsserts
                    .assertThat(sdDocImproveController.getResponse())
                    .isCorrectResponseCode(TrackStudioHttpStatusCodes.HTTP_OK);
        }
