...
public class SlaBug3Test extends BaseIntegrationTest {
    ...
    @Test( groups = {"PROC_SLABUG", "Regression"},description = "запросить информацию", dependsOnMethods = "slaBugMsgStart")
    public void slaBugMsgRequestInfo() {
        apiController.updateToken(generateAuthToken(EMPLOYEE));
        slaBugController.performCommonOperation(task, REQUESTINFO);
    }

    @Test( groups = {"PROC_SLABUG", "Regression"},description = "предоставить решение", dependsOnMethods = "slaBugMsgRequestInfo")
    public void slaBugMsgHotFix() {
        apiController.updateToken(generateAuthToken(EMPLOYEE));
        slaBugController.performCommonOperation(task, HOTFIX);
    }

    @Test( groups = {"PROC_SLABUG", "Regression"},description = "закрыть", dependsOnMethods = "slaBugMsgHotFix")
